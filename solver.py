...
class Solver(object):
    ...
    def get_gen_mols(self, n_hat, e_hat, method):
        ...
        # mols = [self.data.matrices2mol(n_.data.cpu().numpy(), e_.data.cpu().numpy(), strict=False) for e_, n_ in zip(edges_hard, nodes_hard)]
    ...
    def train_or_valid(self, epoch_i, train_val_test='val'):
        ...
        for a_step in range(the_step):
            ...
            edges_noonehot, nodes_noonehot = torch.max(edges_hat, -1)[1], torch.max(nodes_hat, -1)[1]
            edges_uppertri = extract_upper_triangular_no_diag(edges_noonehot, self.device)
            ax_fake_tensor = upper(edges_uppertri, nodes_noonehot)            
