...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dims, z_dim, vertexes, edges, nodes, dropout_rate):
        ...
        self.edges = edges
        self.nodes = nodes
        #this 36/81 comes from extracting the nondiagonal upper triangular values of the adjacency matrix
        self.dropout_rate = dropout_rate * 36 / 81

        self.activation_f = nn.Tanh()
        self.multi_dense_layers = MultiDenseLayers(z_dim, conv_dims, self.activation_f)
        self.edges_layer = nn.Linear(conv_dims[-1], int(vertexes*(vertexes-1)/2))
        self.nodes_layer = nn.Linear(conv_dims[-1], vertexes)
        self.dropout = nn.Dropout(dropout_rate)

    def forward(self, x):
        output = self.multi_dense_layers(x)
        edges_logits = self.edges_layer(output).view(-1, int(self.vertexes*(self.vertexes-1)/2))
        #edges_logits = (edges_logits + edges_logits.permute(0, 1, 3, 2)) / 2
        edges_logits = self.dropout(edges_logits)

        nodes_logits = self.nodes_layer(output)
        nodes_logits = self.dropout(nodes_logits.view(-1, self.vertexes))
